...
int main(int argc, char** argv)
{
	...
	for (unsigned int i = 0; i < playerTypeList.size(); i++)
	{
		...
		if (playerTypeList.at(i) == 0)
		{
			...
			//AI *ai = new AI_1_7(m_track,i,numBoats,1.0,.75,.4);
			AI *ai = new AI_2_0(m_track,i,numBoats,in1,nullptr,0);
			Boat *local_boat = new AIBoat(startPos, *m_world, ai, i, &broadcastList);
