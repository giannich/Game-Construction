...
AI_1_7::AI_1_7(Track *t, int my_player, int num_boats, float width, float acceleratingThreshold, float reversingThreshold)
{
	track = t;
	myPlayer = my_player;
	numBoats = num_boats;
...
InputState AI_1_7::getCommand(std::vector<Boat>& boats,std::vector<Soul>& souls)
{
	...
	int mark = -1;
	int Imin1 = N;
	float dot1 = -std::numeric_limits<float>::infinity();

	for(int i = 0; i < souls.size(); i++)
	{
		if(souls[i].collected == 0)
		{
			vec2 sPos(souls[i].rigidBody->GetPosition().x,souls[i].rigidBody->GetPosition().y);
			if(dist2(sPos,pos)<40*40)
			{
				if(dot(sub(sPos,pos),vel)>0.5*norm(vel)*norm(sub(sPos,pos)))
				{
					for(int j = seg+1; j < seg + 50 && j <= seg1 && j < I; j++)
					{
						if(dot(track->p[j],sPos)+track->c[j] < 0 && dot(track->p[j-1],sPos)+track->c[j-1] >= 0)
						{
							if(j==Imin1)
							{
								if(dot(sub(pos,sPos),vel)>dot1)
								{
									mark = i;
									Imin1 = j;
									dot1 = dot(sub(pos,sPos),vel);
								}
							}
							else
							{
	...
	{
		//std::cout<<"-------------------------------";
		aim = vec2(souls[mark].rigidBody->GetPosition().x,souls[mark].rigidBody->GetPosition().y);
		aim = add(aim, sub(aim,vel));
	}
